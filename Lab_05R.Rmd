...
```{r}
hc_out_new <- hclust(dist(scale(df.new)), method="complete")
plot(hc_out_new, labels=FALSE)
hc_out_new <- hclust(dist(scale(df.new)), method="average")
plot(hc_out_new, labels=FALSE)
# complete is cooler looking, but average seems to be more even
# the plot has unclear usefulness since I can't really tell what the data are supposed to be
```
```
FILL ME IN
```
...
```{r}
clusters <- cutree(hc_out_new, k=5)
clusters
suppressMessages(library(GGally))
ggpairs(df.new, mapping=aes(color=factor(clusters)))
```
```
FILL ME IN
```
